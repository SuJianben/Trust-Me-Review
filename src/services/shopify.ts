...
export async function buildOAuthRedirect(shop: string, env: Env) {
  ...
  const params = new URLSearchParams({ client_id: env.SHOPIFY_API_KEY, scope: "read_products,read_orders,read_fulfillments", redirect_uri: `${env.APP_URL}/auth/callback`, state });
