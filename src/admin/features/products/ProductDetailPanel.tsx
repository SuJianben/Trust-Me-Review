  ...
  return <div className="tmr-product-detail-page">
    ...
    <Card>
      ...
      <div className="tmr-product-stat-cards" role="tablist" aria-label="Product statistics views">
        <div className={activeStat === "total" ? "is-active" : ""} onClick={() => setActiveStat("total")} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") setActiveStat("total"); }} role="tab" aria-selected={activeStat === "total"} tabIndex={0}><Text as="p" tone="subdued">Total reviews</Text><Text as="p" variant="headingLg">{product.trendReviewCount}</Text></div>
        <div className={activeStat === "media" ? "is-active" : ""} onClick={() => setActiveStat("media")} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") setActiveStat("media"); }} role="tab" aria-selected={activeStat === "media"} tabIndex={0}><Text as="p" tone="subdued">Reviews with media</Text><Text as="p" variant="headingLg">{product.trendMediaReviewCount}</Text></div>
        <div className={activeStat === "average" ? "is-active" : ""} onClick={() => setActiveStat("average")} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") setActiveStat("average"); }} role="tab" aria-selected={activeStat === "average"} tabIndex={0}><Text as="p" tone="subdued">Average rating</Text><Text as="p" variant="headingLg">{product.trendAverageRating ? product.trendAverageRating.toFixed(1) : "—"}</Text></div>
      </div>
